                ...
                this.client.on("interactionCreate", async (interaction) => {
                    ...
                    if (interaction.isCommand() &&
                        ...
                        if (nameOption) {
                            ...
                            const user = await this.controller.db.get("SELECT ID from users WHERE ID = ?", (_d = interaction.member) === null || _d === void 0 ? void 0 : _d.user.id);
                            if (user) {
                                await this.controller.db.run("UPDATE users SET NAME = ? WHERE ID = ?", name, (_e = interaction.member) === null || _e === void 0 ? void 0 : _e.user.id);
                            }
                            else {
                                await this.controller.db.run("INSERT INTO users (ID, NAME) VALUES (?, ?)", (_f = interaction.member) === null || _f === void 0 ? void 0 : _f.user.id, name);
                            }
                            interaction.reply({
                                content: `Set ${(_g = interaction.member) === null || _g === void 0 ? void 0 : _g.user.username}'s Name to ${name}`,
                                allowedMentions: { parse: [] },
                            });
                            return;
                        }
