...
class MegalodonClientController {
    ...
    getSSML(message, client, guild, user) {
        ...
        if (content.length > 400 && !(user === null || user === void 0 ? void 0 : user.BYPASS)) {
        ...
        if (content === "tenor.com Link") {
            ...
            content = `${name} sent a ${(_d = message.stickers.first()) === null || _d === void 0 ? void 0 : _d.name} sticker and says ${content}`;
        }
        else if (message.stickers.size > 0) {
            content = `${name} sent a ${(_e = message.stickers.first()) === null || _e === void 0 ? void 0 : _e.name} sticker`;
        }
        else if (((_f = client.guilds.get(guild.id)) === null || _f === void 0 ? void 0 : _f.last_user) !== message.author.id ||
            (((_g = client.guilds.get(guild.id)) === null || _g === void 0 ? void 0 : _g.last_time) || 0) < Date.now() - 180000) {
    ...
    getChannel(user, clientDefault) {
        const channel = this.clients[0].client.channels.cache.find(c => (c instanceof Discord.VoiceChannel || c instanceof Discord.StageChannel) && c.members.has(user.id));
        if (!channel)
            return {};
        let client;
        if (clientDefault) {
